...
public class Nev4 extends Activity {
	...
	public void clickv4(View view) {
		...
		if(random % 8 ==0){
			displayToast("恭喜你随机到：防小狗RUSH！");
		}
		if(random % 8 ==1){
			displayToast("恭喜你随机到：防天鬼！");
		}
		if(random % 8 ==2){
			displayToast("恭喜你随机到：纯鹿流！");
		}
		if(random % 8 ==3){
			displayToast("恭喜你随机到：雄鹿+角鹰！");
		}
		if(random % 8 ==4){
			displayToast("恭喜你随机到：角鹰+QML！");
		}
		if(random % 8 ==5){
			displayToast("恭喜你随机到：吹风流或爆ACRUSH！");
		}
		if(random % 8 ==6){
			displayToast("恭喜你随机到：其他大招山岭拆家流！");
		}
		if(random % 8 ==7){
